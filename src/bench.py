...
API_URL = "http://127.0.0.1:8000/api/male/{z}/{y}/{x}"
...
async def fetch_tile(client, z, y, x):
    url = API_URL.format(z=z, y=y, x=x)
...
async def main(concurrent_requests=20):
    async with httpx.AsyncClient() as client:
        ...
        print(f"Total time for {concurrent_requests} requests: {end_time - start_time:.2f} seconds")
        print(f"Successful responses: {success_count}/{concurrent_requests}")

if __name__ == "__main__":
    asyncio.run(main(concurrent_requests=20))
